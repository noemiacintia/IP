'''Você agora é responsável por implementar algumas funcionalidades para um serviço de playlist de músicas. Escreva um programa que recebe um número inteiro n≥2, que indicará a quantidade de músicas a serem lidas, e logo em seguida, o programa irá ler n músicas juntamente com o seu tempo de duração. As músicas serão fornecidas no seguinte formato:

Nome_Da_Musica MM:SS

onde MM:SS é o tempo de duração da música, exemplo: 01:30. Assuma que o Nome_Da_Musica será sempre uma palavra única, sem espaços. Assuma também que não existirão músicas com duração idêntica nessa playlist.
...
DICA: Para a resolução dessa questão, é recomendável a utilização das funções datetime e timedelta da biblioteca datetime

 

Entrada 
O valor n (n≥2), e, em seguida, n entradas, uma por vez, no formato: Nome_da_Musica MM:SS

 

Saída
Seu programa deve mostrar na saída os seguintes dados, nesta ordem (ver exemplos):

Nome da música de maior duração
Nome da música de menor duração
Tempo total de duração da playlist (no formato especificado anteriormente)
